...
### [ParseTransform](https://github.com/OceanDataTools/openrvdas/blob/master/logger/transforms/parse_transform.py)
  ...
  If ```return_json=True```, output a JSON-encoded string for the dict; if ```return_das_record=True```, return a [DASRecord](https://github.com/OceanDataTools/openrvdas/blob/master/logger/utils/das_record.py) for it. See [Parsing](/parsing/) for more details.
